...
from .agent_SAET import TEAChAgentSubgoalAwareET
...
class TEAChAgentOracleNavigator:
    ...
    def load_navigator(self):
        self.navigator = OracleNavigator(self.logger)
        self.logger.info(f"Oracle navigator is loaded")
    ...
    def reset(self, edh_instance, edh_history_images, edh_name=None, env=None):
    ...
    def predict_controller_action(
        ...
        if not visible_objs:
            ...
            self.logger.debug(repr(frame_np))
            # if self.plot_and_pause:
            #     curr_cv2 = cv2.cvtColor(frame_np, cv2.COLOR_BGR2RGB)
            #     cv2.imshow("current obs %d" % self.agent_id, curr_cv2)
            #     cv2.waitKey(0)
        return proposals
    ...
    def get_navigator_action(self, target_object_type):
        action_name = self.navigator.step(target_object_type)
        pred_action = (action_name, -1, 1.0)

        agent_action = ETAgentAction(
            action_type="navigation",
            proposals=[pred_action],
            name=pred_action,
            idx_in_proposal=0,
        )
        self.agent_state.record_action(agent_action)
    ...
    def step(self, frame):
        ...
        if self.agent_state.is_navigating:
            agent_action = self.get_navigator_action(self.target_object_type)
        else:
    ...
    def forward_one_more_step(self, frame, agent_action):
        ...
        if self.agent_state.is_navigating and agent_action.name[0] in ["Stop", "[END]"]:
            self.logger.debug(
            ...
            self.target_object_type = agent_action.arg1[0]
            agent_action = self.get_navigator_action(self.target_object_type)
        else:
    ...
    def navigation_error_handling(self):
        last_action = self.agent_state.get_last_action()
        # action_name = random.choice(["RotateLeft", "RotateRight"])
        # new_action = (action_name, -1, 1.0)
        new_agent_action = ETAgentAction(
            action_type="navigation",
            proposals=last_action.proposals,
            name=self.end_navi_action,
            idx_in_proposal=-1,
        )
