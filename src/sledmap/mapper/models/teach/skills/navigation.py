...
class NavigationSkill:
    ...
    def __init__(self, logger=None, disable_search_hints=False):
        self.logger = logger
        self.log_func = logger.info
        self.disable_search_hints = disable_search_hints
    ...
    def navigate_to(self, target_instance, state_repr: NeuralSymbolicAgentState, force_set_goal=False):
        # try to move closer
        # if self.target_instance is None:

        interactable = target_instance.state.interactable()
        if interactable:
            self.log_func("Reach the target instance: %r" % target_instance)
            self.target_instance = None
            self.goto_instance_skill.reset()
            return TeachAction.stop_action()

        if self.target_instance is None or force_set_goal:
            self.log_func("=====> Set navigation goal: %r" % target_instance)
            self.target_instance = target_instance
            self.goto_instance_skill.set_goal(target_instance)
        
        self.log_func(self.goto_instance_skill.status_summary(state_repr.get_agent_pos_m()))
        action = self.goto_instance_skill.act(state_repr)
        self.log_func("Get a navigation action: %r" % action)
    ...
    def search_target(self, search_types: set, state_repr: NeuralSymbolicAgentState):
        self.log_func("Searching for %r:"%search_types)
        ...
        if not self.is_searching:
            ...
            for instance_id in state_repr.get_all_instance_ids():
                instance = state_repr.get_instance_by_id(instance_id)
                if instance.object_type in search_types:
                    instance_ids_of_interest.append(instance_id)
            if not self.disable_search_hints: 
                hints = state_repr.get_search_hints(search_types)
                print("########### Search Hints: %r" % hints)
            else:
                hints = []
            self.search_skill.set_goal(
                search_types, instance_ids_of_interest, hints  # TODO: add hint list
            )
            self.is_searching = True

        action = self.search_skill.act(state_repr)
        self.log_func(self.search_skill.status_summary(state_repr.get_agent_pos_m()))
        ...
        if interactable or action.is_stop() or action.is_failed():
            ...
            if action.is_failed():
                self.log_func("Do not find any instance for: %r" % search_types)
            if interactable:
