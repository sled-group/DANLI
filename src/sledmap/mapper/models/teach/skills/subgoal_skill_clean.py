...
class SubgoalSkillClean(NavigationSkill):
    def __init__(
        self,
        logger=None,
    ):
        ...
        self.log_func("SubgoalSkillClean is initialized!")
        self.failure_num = 0
    ...
    def reset(self):
        ...
        self.failure_num = 0
        self.STATE = "INIT"
        self.log_func("SubgoalSkillClean is reset!")
    ...
    def step(
        ...
        for i in range(10):
            ...
            if self.STATE == "INIT":
                ...
                candidates = []
                for instance in state_repr.get_all_instances():
                    if check_obj_X_is_Y(instance.object_type, self.target_type):
                        if instance.state["isDirty"].get_value() and any([o in instance.state["parentReceptacles"].get_values() for o in WaterBottomReceptacles]):
                            self.target_instance = instance
                            self.STATE = "TOGGLE_WATER"
                            break
                        elif instance.state["isDirty"].get_value():
                            self.target_instance = instance
