...
from sledmap.mapper.flags import GLOBAL_VIZ

GRID_SIZE = 61

TERMINAL_OBSTACLE_REWARD = -0.9    # Reward for colliding with obstacles
TERMINAL_STOP_GOAL_REWARD = 2.0    # Reward for stopping at the goal position
TERMINAL_STOP_ALL_REWARD = 0.0   # Reward for stopping at a non-goal position.
...
class GoToSkill(Skill):
    def __init__(self):
        ...
        self.select_gridaction_times = 0
    ...
    def act(self, state_repr: TeachSpatialStateRepr) -> TeachAction:
        ...
        # print("=======================> GOTO SKILL ACT")
        # print("prev_pos", self.prev_pos)

        q_image = self.vin.compute_q_image()

        if q_image.isnan().any():
            print('There is nan in q_image, stop!') # TODO: avoid this
        ...
        VIZ = ANNOTATE_FOR_TREE_VIZ = GLOBAL_VIZ
        VIZ = True
        ...
        if ANNOTATE_FOR_TREE_VIZ:
            ...
            state_repr.annotations["reward_map"] = self.vin.rewardmap
        ...
        if VIZ:
            ...
            # annotate agent trajectory
            if state_repr.annotations.get("agent_trajectory"):
                for ax, ay, az in state_repr.annotations["agent_trajectory"][:-1]:
                    x_vx_q_pv, y_vx_q_pv = self.vin.remap_xy(ax, ay, state_repr.data.data.shape[2])
                    x_vx_q_pv, y_vx_q_pv = max(min(x_vx_q_pv, vh-1), 0), max(min(y_vx_q_pv, vw-1), 0)
                    v_image_c[:, x_vx_q_pv, y_vx_q_pv] = torch.tensor([0, 0, 1])
            
            # annotate agent current position
            v_image_c[:, x_vx_q, y_vx_q] = 0 # torch.tensor([0, 0, 1])


            # # Print top5 values for debugging
            _top5_v_values = sorted(v_image_c_pos.view(-1).detach().cpu().numpy().tolist(), reverse=True)[:5]
            # print(f"VIN Top5 V_image values: {_top5_v_values}")

            # visualize target position
            # print(torch.where(self.vin.rewardmap > 0.99))
            tgt_x_vx, tgt_y_vx = [i for i in torch.where(self.vin.rewardmap > 0.95)[2:]]
            v_image_c[:, tgt_x_vx, tgt_y_vx] = 1.0

            v_image_c_to_show = show_image(v_image_c, "VIN Value Image", scale=12, waitkey=None)
            state_repr.annotations["navigation_map"] = v_image_c_to_show
