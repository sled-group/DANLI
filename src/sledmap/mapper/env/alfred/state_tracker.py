...
from sledmap.mapper.env.privileged_info import PrivilegedInfo

from sledmap.mapper.models.alfred.hlsm.alfred_perception_model import AlfredSegmentationAndDepthModel

import sledmap.mapper.paths
...
class StateTracker():
    ...
    def __init__(self,
        ...
        if self.reference_seg:
            ...
            self.seg_model.load_state_dict(torch.load(sledmap.mapper.paths.get_segmentation_model_path()))
            self.seg_model.eval()
        if self.reference_depth:
            self.depth_model = None
        else:
            self.depth_model = AlfredSegmentationAndDepthModel(hparams).to(PERCEPTION_DEVICE)
            self.depth_model.load_state_dict(torch.load(sledmap.mapper.paths.get_depth_model_path()))
