...
class Model(base.Model):
    def __init__(self, args, embs_ann, vocab_out, pad, seg):
        ...
        self.decoder = nn.TransformerDecoder(
            decoder_layer, args.decoder_lang['layers'])
        self.enc_pos = PosLangEncoding(args.demb) if args.decoder_lang[
            'pos_enc'] else None
    ...
    def encode_vl(self, vocab, **inputs):
        ...
        if lang is not None:
            ...
            length_lang_max = inputs['length_lang_max']
        else:
            emb_lang = torch.zeros([batch_size, 0, self.args.demb]).to(device)
            lengths_lang, length_lang_max = torch.tensor([0] * batch_size), 0
    ...
    def encode_lang(self, vocab, lang_pad):
        ...
        emb_padding = torch.zeros(
            emb_lang.shape[:2], device=emb_lang.device).bool()
    ...
    def forward(self, vocab, **inputs):
        ...
        target_mask = model_util.triangular_mask(
            lang_target.size(1), lang_target.device)
        ...
        output = {'lang': lang_out_flat.view(
            len(decoder_out), -1, lang_out_flat.shape[-1])}
        return output
    ...
    def compute_metrics(self, model_out, gt_dict, metrics_dict, verbose=False):
        ...
        pred_lang = model_util.tokens_to_lang(
            pred_tokens, self.vocab_out, {self.pad}, join=False)
        gt_lang = model_util.tokens_to_lang(
            gt_dict['action'], self.vocab_out, {self.pad}, join=False)
        pred_lang_strs = [' '.join(s) for s in pred_lang]
        gt_lang_strs = [' '.join(s) for s in gt_lang]
        model_util.compute_f1_and_exact(
            metrics_dict, pred_lang_strs, gt_lang_strs, 'lang')
        if verbose:
            print('Lang GT:\n{}'.format(gt_lang_strs[0]))
            print('Lang predictions:\n{}'.format(pred_lang_strs[0]))
            print('EM = {}, F1 = {}'.format(metrics_dict['lang/exact'][-1],
                                            metrics_dict['lang/f1'][-1]))

    def translate(
            self, vocab_in, max_decode=300, num_pad_stop=3, verbose=False, **inputs):
        '''
        ...
        for i in range(max_decode):
            ...
            emb_cur = self.enc_pos(emb_cur) if self.enc_pos else target
            ...
            if len(tokens_out[0]) > num_pad_stop and (
                    np.array(lang_cur)[:, -num_pad_stop:] == self.pad).all():
                break

        lang_result = [l[1:] for l in lang_cur]
        lang_result = [[t for t in tokens if t != self.pad] for tokens in lang_result]
        if verbose:
            print(model_util.tokens_to_lang(lang_result[0], vocab_translate))
        return lang_result
