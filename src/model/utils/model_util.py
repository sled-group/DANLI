...
def test_extractor(orig_json_path, extractor, feats_orig):
    ...
    if not images_root.is_dir():
        print(
            "WARNING: feature extraction unit check can not be performed, directory does not exist"
        )
        return
    first_image = Image.open(images_root / "000000000.png")
    feat_extracted = extractor.featurize([first_image], batch=1)
    assert torch.isclose(
        feat_extracted.mean(), feats_orig[0].mean()
...
def generate_attention_mask(len_lang, len_frames, device, num_input_actions=0):
    ...
    frames_to_actions = torch.ones(
        (len_frames, len_frames), device=device
    ).float() * float("-inf")
...
def get_causal_mask(orderings, device=None):
    """
    generate causal mask for attention (token of order t does not attend to tokens after t)
    params: ordering: tensor of size (B, T) that contains the order of each step
    return: bool matrix of size (B, T, T) where the allowed attention positions are False
    """
    mask_idx = orderings[:, None, :] > orderings[:, :, None]
    # B, T = orderings.shape
    # mask = torch.zeros((B, T, T), device=device)
    # mask = mask.masked_fill(~mask_idx, float('-inf'))
    return mask_idx


def get_pad_mask(multiple_input_lengths, device=None):
    """
    Generate pad masks for inputs.
    Note that the padding positions should be "True".

    multiple_input_lengths: list of lengths of size BatchSize
                            E.g., [lengths_lang, lengths_traj, lengths_traj]
    return: pad mask of size (B, length_lang_max + 2*length_traj_max)
    """

    mask_pad = []
    for lengths in multiple_input_lengths:
        maxlen = lengths.max()
        try:
            ...
            print("\n\n\n get_pad_mask")
            print(
                [(lengths.shape, lengths.device) for lengths in multiple_input_lengths]
            )
        mask_pad.append(mask)

    return torch.cat(mask_pad, dim=1)
