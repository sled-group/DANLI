...
from teach.utils import (
    ...
    load_json,
    update_objs_with_custom_metadata,
)
...
def evaluate_traj(
    ...
    if total_goal_conditions != 0:
        ...
        goal_condition_success_rate = 1.0 - (
            unsatisfied_goal_conditions / total_goal_conditions
        )
    else:
    ...
    return {
        "completed_goal_conditions": int(init_gc_total - init_gc_satisfied)
...
def aggregate_metrics(traj_stats, args):
    ...
    num_fails_by_api_limit = sum(
        [v["num_api_fails"] >= args.max_api_fails for k, v in traj_stats.items()]
    )
    ...
    if total_path_len_weight > 0.0 and not np.isclose(total_path_len_weight, 0.0):
        plw_sr = (
            float(
                sum([v["path_len_weighted_success_spl"] for k, v in traj_stats.items()])
            )
...
def load_edh_traj_metrics(output_file, pred_actions_file, args):
    if args.split in [
        ...
        split = args.split
    elif args.split in ["divided_val_seen", "divided_test_seen"]:
        split = "valid_seen"
    elif args.split in ["divided_val_unseen", "divided_test_unseen"]:
        split = "valid_unseen"

    with open(output_file) as h:
        inference_game_json = json.load(h)
    ...
    final_state_objects = update_objs_with_custom_metadata(
        inference_episode["final_state"]["objects"],
        inference_episode["final_state"]["custom_object_metadata"],
    )
    ...
    traj_metrics = create_new_traj_metrics(
        edh_instance["instance_id"], edh_instance["game_id"]
    )
    ...
    # NAVI_ACTIONS = {"Forward", "Backward", "Turn Left", "Turn Right", "Pan Left", "Pan Right", "Look Up", "Look Down", "Stop"}
    # all_actions_are_navi = all([a['action'] in NAVI_ACTIONS for a in pred_actions])
    # if all_actions_are_navi:
    #     print(output_file)
    #     traj_metrics["goal_condition_success"] = 0.0
    #     traj_metrics["success_spl"] = 0.0
    #     traj_metrics["path_len_weighted_success_spl"] = 0.0
    #     traj_metrics["goal_condition_spl"] = 0.0
    #     traj_metrics["path_len_weighted_goal_condition_spl"] = 0.0
    #     traj_metrics["success"] = 0
    #     traj_metrics["completed_goal_conditions"] = 0
...
def load_tfd_traj_metrics(output_file, pred_actions_file, args):
    ...
    tfd_instance_file = os.path.join(
        args.data_dir, "tfd_instances", args.split, tfd_instance_fn
    )
    ...
    init_state_objects = update_objs_with_custom_metadata(
        inference_episode["initial_state"]["objects"],
        inference_episode["initial_state"]["custom_object_metadata"],
    )
