...
def apply_state_diff(state, state_diff):
    ...
    obj_changes_remaining = set(state_diff["objects"].keys()).difference(
        obj_changes_applied
    )
    logger.debug(
        "Objects to be changed that involved slicing / cracking :"
    ...
    logger.debug(
        "Cur objects :"
        + str([(idx, obj["objectId"]) for (idx, obj) in enumerate(state["objects"])])
    )
    ...
    logger.debug(
        "Objects after deletion :" + str([obj["objectId"] for obj in state["objects"]])
    )
    return state
...
def create_task_thor_from_state_diff(state_diff):
    ...
    for obj_id in state_diff["objects"]:
        ...
        props_for_task = set(state_diff["objects"][obj_id].keys()).intersection(
            props_to_check
        )
        overridden_props = [
            prop_overrides[prop]
