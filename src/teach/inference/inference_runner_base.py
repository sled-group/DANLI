...
class InferenceRunnerBase:
    ...
    def _launch_processes(self, input_files, config: InferenceRunnerConfig):
        ...
        try:
            for process_index in range(config.num_processes):
                simulator_options = {
                    'renderInstanceSegmentation': '--hlsm_use_gt_obj_det' in config.model_args,
                    'renderSemanticSegmentation': '--hlsm_use_gt_seg' in config.model_args,
                    'renderDepthImage': '--hlsm_use_gt_depth' in config.model_args
                }
                logger.info("Simulator rendering options: %r"%simulator_options)
                er = EpisodeReplay(
    ...
    def _run(self, process_index, files_to_process, config: InferenceRunnerConfig, er: EpisodeReplay):
        ...
        model = config.model_class(
            process_index, config.num_processes, model_args=config.model_args
        )
    ...
    def _run_instance(self, instance_file, config: InferenceRunnerConfig, model: TeachModel, er: EpisodeReplay):
        ...
        try:
            ...
            init_success = False
            logger.error(
                f"Failed to initialize episode replay for instance={instance_id}",
                exc_info=True,
            )
        ...
        try:
            # model_started_success = model.start_new_edh_instance(
            #     edh_instance, edh_history_images, instance_file
            # )
            gt_histrory_perceptions = er.gt_history_perceptions if er.record_events else None
            # if not gt_histrory_perceptions:
            #     model_started_success = model.start_new_edh_instance(
            #         edh_instance, edh_history_images, instance_file
            #     )
            # else:
            model_started_success = model.start_new_edh_instance_cheating(
                instance, history_images, instance_file, er.simulator.controller, gt_histrory_perceptions
            )
        except Exception:
            model_started_success = False
            metrics["error"] = 1
            logger.error(
                f"Failed to start_new_edh_instance for {instance_id}", exc_info=True
            )
        ...
        if model_started_success:
            ...
            for _ in range(config.max_traj_steps):
                ...
                try:
                    ...
                    # image_name = InferenceRunnerBase._save_image(
                    #     config, instance, img, traj_steps_taken
                    # )
                    image_name=None
                    action, obj_relative_coord = model.get_next_action(
                        img, instance, prev_action, image_name, instance_file
                    )
                    step_success = InferenceRunnerBase._execute_action(
                        er.simulator, action, obj_relative_coord
                    )
                    InferenceRunnerBase._update_metrics(
                        metrics, action, obj_relative_coord, step_success
                    )
                    prev_action = {
                        "action": action,
                    ...
                    metrics["error"] = 1
                    break
                if InferenceRunnerBase._should_end_inference(
                    action, metrics, config.max_api_fails
                ):
                    break
        ...
        output_file = os.path.join(
            config.output_dir, "inference__" + instance_id + ".json"
        )
    ...
    def _get_metrics_file_name_for_process(process_index, metrics_file):
        return f"{metrics_file}.{process_index}.json"
