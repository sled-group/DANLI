...
class EpisodeReplay:
    def __init__(self, simulator_name, live_feeds, process_index=None, configs={}):
        """
        ...
        try:
            ...
            gpu_id = 0 if process_index is None else process_index % gpu_cnt
            x_display = f"{os.environ['DISPLAY'].split(':')[1]}.{gpu_id}"
        except:
            raise Exception("DISPLAY environment variable not set")

        self.simulator = simulator_factory.factory.create(
            simulator_name, web_window_size=900, x_display=x_display, **configs
        )
        ...
        self.record_events = any(configs.values())
    ...
    def set_episode_by_fn_and_idx(self, game_fn, task_idx, episode_idx):
        ...
        structured_log = Dataset.import_json(
            file_name=game_fn, process_init_state=False, version="2.0"
        )
    ...
    def play_episode(
        ...
        if (
            not force_replay
            and obs_dir is not None
            and os.path.isdir(obs_dir)
            and len(os.listdir(obs_dir)) > 0
        ):
            logger.warn(
                "play_episode skipping playback in non-empty dir '%s'" % obs_dir
            )
            return False, False

        api_success, init_state = self.set_up_new_episode(obs_dir, turn_on_lights, task)
        init_state_objects = self.simulator.get_objects()

        self.gt_history_perceptions = []
        ...
        self._write_last_states_and_frames(
            init_state, obs_dir, target_object_active, write_frames, write_states
        )

        _, task_success, _, _, _ = self.simulator.check_episode_progress(
            self.simulator.current_task
        )
    ...
    def stitch_episode_video(self, obs_dir, font_fn, force_replay=False):
        ...
        if not force_replay and os.path.isdir(out_dir) and len(os.listdir(out_dir)) > 0:
            logger.warn(
                "stitch_episode_video skipping video stitch in non-empty dir '%s'"
                % out_dir
            )
            return
        ...
        for fn in frame_fns:
            if fn.split(".")[-1] == "jpeg":
                ...
                frame = Image.new(
                    "RGB",
                    (
                        frame_border * 2 + ref_im.width * 4,
                        frame_border * 2 + ref_im.height * 2,
                    ),
                )
                break
        frame_layout = {
        ...
        for t in tqdm.tqdm(
            sorted(
                timestamps_to_fns,
                key=lambda x: float(x) if x and x != "end" else float("inf"),
            )
        ):
                ...
                if frame_type in frame_layout:  # visual observation
                    ...
                    draw.rectangle(
                        (
                            ...
                            (
                                frame_border
                                + ref_im.width * frame_layout["bottomright"][0],
                                frame_border
                                + ref_im.height * frame_layout["bottomright"][1],
                            ),
                        ),
                        fill="black",
                    )
                    ...
                    while len(s) > 0:
                        draw.text(
                            (
                                frame_border
                                + ref_im.width * frame_layout["text"][0]
                                + 5,
                                frame_border
                                + ref_im.height * frame_layout["text"][1]
                                + (font_size + 5) * line_idx,
                            ),
    ...
    def write_progress(self, frame_idx, obs_dir):
        progress_check_output = self.simulator.current_task.check_episode_progress(
            self.simulator.get_objects(self.simulator.controller.last_event),
            self.simulator,
        )
        with open(
            os.path.join(
    ...
    def write_frames(self, frame_idx, obs_dir, target_object_active):
        ...
        self._write_frame(
            frames["targetobject"]
            if target_object_active
            else np.zeros_like(frames["targetobject"]),
            os.path.join(
                obs_dir,
    ...
    def _play_single_interaction(
        ...
        # customizing for recording ground truth semantic segmentation and depth image
        interaction = self.episode.interactions[idx]
        if (
            self.record_events
            and interaction.agent_id == 1
            and interaction.action.success == 1
        ):
            self.gt_history_perceptions.append(
                copy.deepcopy(self.simulator.controller.last_event)
            )
        # from sledmap.mapper.utils.viz import show_image, show_image_with_bbox_and_point
        # if hasattr(interaction.action, 'x'):
        #     point = (interaction.action.x * 900, interaction.action.y * 900)
        # else:
        #     point = None
        # waitkey = 0 if idx >=82 else None
        # show_image_with_bbox_and_point(self.simulator.get_latest_images()["ego"], [], point=point, waitkey=None, name='ego')
        # show_image(self.simulator.controller.last_event.depth_frame, name='depth')
        # show_image(self.simulator.controller.last_event.instance_segmentation_frame, name='semantic')

        action_definition = action_id_to_info[
            self.episode.interactions[idx].action.action_id
        ]
        logger.debug(
            "taking action <<%s, %s>>"
            % (action_definition["action_type"], action_definition["action_name"])
        )
        ...
        if logged_success == 1:
            ...
            api_success = api_success & (
                1 == self.episode.interactions[idx].action.success
            )
        ...
        # print("=====> [%d] _play_single_interaction"%idx, api_success, target_object_active, self.episode.interactions[idx].action.success)
    ...
    def _add_interaction(self, idx, interact_oid, logged_success):
        ...
        if interact_oid is not None and (
            ...
            # Note: if the interacted oid is not the same as logged, we have nothing to do since the interaction is
            # already happened. We should not add this condition here cuz the following exception handling process
            # does not help with that. We should continue on the "wrong" object, while in most cases the agent just
            # picked another piece of slice (incorrectly picked due to they are so close). It is okay actually.
        ):
            # Next, override the provided user (x, y) with a randomly selected mask point (x*, y*) on the object.
            if not self.record_events:
                # the original TEACh code
                mask_frame = self.simulator.get_target_object_seg_mask(interact_oid)[
                    "mask"
                ]
            else:
                # when semantic segmentation or depth image rendering turned off, sometime there is bug in instance mask
                # while the TEACh data is collected with them turned off, the interact point may not be usable when we
                # turns these ground truth rendering on.
                # Here we try to fix the this by using "instance_masks" instead of "instance_segmentation_frame"
                # Besides, there is another bug here. When the agent picks up a wrong object (different oid),
                # we need to use the logged oid instead of the just interacted one.
                if not self.simulator.renderSemanticSegmentation:
                    ac = dict(action="Pass", agentId=0, renderObjectImage=True)
                    self.simulator.controller.step(ac)
                mask_frame = self.simulator.controller.last_event.instance_masks[
                    interact_oid
                ]
            mask_points = np.where(mask_frame == 1)[:2]
            if (
                len(mask_points[0]) > 0
            ):  # if any part of the object is visible in the frame, pick a point on it
                rpoint_idx = np.random.randint(0, len(mask_points[0]))
                override_interaction = copy.deepcopy(self.episode.interactions[idx])
                override_interaction.action.x = (
                    mask_points[1][rpoint_idx] / self.simulator.web_window_size
                )
                override_interaction.action.y = (
                    mask_points[0][rpoint_idx] / self.simulator.web_window_size
                )
                logger.info(
                    "... override interaction %s" % override_interaction
                )  # DEBUG

                # from sledmap.mapper.utils.viz import show_image, show_image_with_bbox_and_point
                # if hasattr(override_interaction.action, 'x'):
                #     point = (override_interaction.action.x * 900, override_interaction.action.y * 900)
                # else:
                #     point = None
                # show_image(mask_frame.astype(int), name='mask')
                # show_image_with_bbox_and_point(self.simulator.get_latest_images()["ego"], [], point=point, waitkey=0, name='ego1')

                self.simulator.add_interaction(override_interaction)
                # if self.episode.interactions[idx].action.success != logged_success: #bug in original teach code
                if override_interaction.action.success != logged_success:
                    logger.info(
                        "...... action success logged %d != %d of override action with oid %s just taken"
                        % (
                            logged_success,
                            override_interaction.action.success,
                            interact_oid,
                        )
                    )
                self.episode.interactions[idx] = override_interaction
                ...
                logger.info(
                    "Cur objs (ID, visible): %s "
                    % str([(obj["objectId"], obj["visible"]) for obj in cur_objs])
                )
                self.simulator.add_interaction(
                    self.episode.interactions[idx], on_oid=interact_oid, force=True
                )
                if self.episode.interactions[idx].action.success != logged_success:
                    logger.info(
    ...
    def set_up_new_episode(self, obs_dir=None, turn_on_lights=False, task=None):
        ...
        self.simulator.start_new_episode(
            ...
            if self.episode.commander_embodied == "True"
            else False,
        )
        logger.info("... done")
        ...
        if task is not None:
            ...
            logger.info(
                "Setting task %s with task_params %s..." % (self.task, self.task_params)
            )
            self.simulator.set_task_by_name(
                task_name=self.task, task_params=self.task_params
            )
        logger.info("... done")
        ...
        init_state = reduce_float_precision(
            self.simulator.get_current_state().to_dict()
        )
    ...
    def _write_last_states_and_frames(
        self, init_state, obs_dir, target_object_active, write_frames, write_states
    ):
        frame_idx = "end"
        if obs_dir is not None and write_states:
            self.write_cur_state(frame_idx, obs_dir, init_state)
    ...
    def _write_progress_check(
        self, idx, obs_dir, action_definition, target_object_active
    ):
        frames = self.simulator.get_latest_images()
        ...
        if action_definition["action_name"] == "OpenProgressCheck":
            with open(
                ...
                json.dump(
                    r, f
                )  # Success, subgoal success, string descriptions, problem objects
        ...
        # a segmentation mask + actives the target object camera
            ...
            if "shown_oid" in r and len(r["shown_oid"]) > 0:
                mask_points = np.where(
                    self.simulator.get_target_object_seg_mask(r["shown_oid"])["mask"]
                    == 1
                )[:2]
